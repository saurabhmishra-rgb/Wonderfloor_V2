...
//                       </span>
//                       <span className={`text-[10px] md:text-xs ${dm.footerSub}`}>{selectedProduct.size}</span>
//                     </div>
...
//                 >
//                   <img src={selectedProduct.img} alt="Selected"
  ...
  const productCategories = Array.from(new Set(
    combinedProducts
      ...
      .filter(p => {
        ...
        //  NAYA: Check agar KOI BHI filter selected hai (Colour, Thickness, Collection etc.)
        const hasAnyFilterActive = Object.values(activeFilters).some(arr => arr.length > 0);

        //  UPDATED: Room restriction ko bypass karo agar search active hai YA koi filter active hai
        return !hasRoomFilter || roomSupportedCollections.includes(p.accordionCategory) || hasAnyFilterActive || isSearching;
      })
      .map(p => p.accordionCategory)
  ));
...
useEffect(() => {
  ...
  const filteredProducts = combinedProducts.filter(prod => {
    ...
    // ✅ NAYA: Check agar KOI BHI filter selected hai
    const hasAnyFilterActive = Object.values(activeFilters).some(arr => arr.length > 0);

    // ✅ UPDATED: Ab yeh condition har filter (Thickness, Colour, etc.) ke liye restriction hatayegi
    if (hasRoomFilter && !roomSupportedCollections.includes(prod.accordionCategory) && !hasAnyFilterActive && !isSearching) {
      return false;
    }
