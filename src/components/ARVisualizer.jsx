...
const ARVisualizer = ({ closeModal, initialImage }) => {
  ...
  useEffect(() => {
    let timer;
    if (activeBaseImage?.maskUrl && threeContainerRef.current) {
      timer = setTimeout(() => {
    ...
    return () => {
      ...
      if (visualizerInstance.current && visualizerInstance.current.cleanup) {
                              ...
                              {categoryProducts.map((prod) => {
                                ...
                                return (
                                  <div
                                    ...
                                    className={`relative flex gap-3 md:gap-4 p-2 md:p-3 border rounded-lg cursor-pointer transition-all duration-300 bg-white ${selectedProduct.id === prod.id ? 'border-[#0b5e5e] shadow-md bg-[#0b5e5e]/5 transform scale-[1.02]' : 'border-gray-200 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:-translate-y-1'}`}
                                  >
                              ...
                              {categoryProducts.map((prod) => {
                                ...
                                return (
                                  <div
                                    key={prod.id}
                                    onClick={() => handleTileSelection(prod)}
                                    className={`relative aspect-square rounded overflow-hidden cursor-pointer border-2 transition-all duration-300 bg-white ${selectedProduct.id === prod.id ? 'border-[#0b5e5e] shadow-lg transform scale-105 z-10' : 'border-transparent hover:border-gray-200 hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:scale-105'}`}
                                  >
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh] overflow-hidden transform transition-all animate-fade-in-up">
            ...
            <div className="p-4 sm:p-5 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0">
              ...
              <button
                onClick={() => { handleTileSelection(detailsProduct); setIsDetailsModalOpen(false); }}
