...
function intersectImagePixelWithFloorPlane(u, v, cameraData, cvNormal, planeDistance, maxDist = Infinity) {
...
function buildCalibratedFloorGeometry(prediction) {
  ...
  const planeDistance = Number.isFinite(plane.distance) ? plane.distance : cameraData.height_meters;

  const threePlaneNormal = new THREE.Vector3(cvNormal.x, -cvNormal.y, -cvNormal.z).normalize();
  ...
  const depthAxis = new THREE.Vector3().crossVectors(threePlaneNormal, widthAxis).normalize();

  let planeOrigin = intersectImagePixelWithFloorPlane(cameraData.cx, cameraData.cy, cameraData, cvNormal, planeDistance);
  if (!planeOrigin) {
  ...
  const makePoint = (w, d) => planeOrigin.clone().addScaledVector(widthAxis, w).addScaledVector(depthAxis, d);
...
export const initVisualizer = (container, predictionData = null) => {
  ...
  try {
    const scene = new THREE.Scene();
    const camera = new THREE.PerspectiveCamera(50, container.clientWidth / container.clientHeight, NEAR, FAR);
    ...
    const prediction = predictionData?.runpod?.output?.prediction || predictionData?.output?.prediction || predictionData;

    const inputData = prediction?.images?.input || prediction?.input;

    if (prediction?.camera && prediction?.floor) {
      configureCameraFromIntrinsics(camera, prediction.camera, inputData, container.clientWidth, container.clientHeight);
